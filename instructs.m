...
WaitSecs(2);
  ...
  disp('2b')
        ...
        trial=1;
        time = visual_feedback(window,1,color,where,sounds,feedback,stim,time,trial,trialWithinBlock,trainTest);
  disp('2c')

        %feedback following incorrect trial
        displayText(window,'instructs3.txt',color,where,stim); while(KbCheck);end; KbWait;
        trial=2;
        time = visual_feedback(window,0,color,where,sounds,feedback,stim,time,trial,trialWithinBlock,trainTest);

  disp('2d')
        %wrong key
        displayText(window,'instructs4.txt',color,where,stim); while(KbCheck);end; KbWait;
        trial=2;
        time = visual_feedback(window,2,color,where,sounds,feedback,stim,time,trial,trialWithinBlock,trainTest);
disp('2e')
        ...
        trial=2;
        time = visual_feedback(window,99,color,where,sounds,feedback,stim,time,trial,trialWithinBlock,trainTest);
disp('2f')
